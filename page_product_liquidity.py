...
def render_product_table_fragment(selected_product, df_processed, display_options_full):
    ...
    if _from_session:
        ...
        if "期末余额" in df.columns:
            df["期末余额"] = (df["现金流入"].fillna(0) - df["现金流出"].fillna(0)).cumsum().round(2)
    ...
    # Recalculate balance (vectorized)
    if "期末余额" in edited_df.columns:
        edited_df["期末余额"] = (edited_df["现金流入"].fillna(0) - edited_df["现金流出"].fillna(0)).cumsum().round(2)
...
# ---------- 未来待办提醒相关函数 ----------
def get_upcoming_events_from_sheets(sheets_data, n_days=5):
    """从每个产品的流动性表中获取未来N天的待办事件"""
    ...
    events.sort(key=lambda x: (x["日期"], x["产品"]))
    return events


def render_upcoming_reminders(sheets_data):
    """渲染未来5天待办提醒（从各产品流动性表读取）"""
    events = get_upcoming_events_from_sheets(sheets_data)
    ...
    inflow_count = sum(1 for e in events if e["现金流入"] > 0)
    outflow_count = sum(1 for e in events if e["现金流出"] > 0)
    ...
    <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:6px;
        ...
        📅 <b>未来5日待办</b> · {total_count}笔 · 📥{inflow_count} 📤{outflow_count} {badge_html}
    </div>
    """, unsafe_allow_html=True)
        ...
        for event_date in sorted(events_by_date.keys()):
            ...
            for e in day_events:
                if e["现金流入"] > 0:
                    ...
                    op = "📤"
                    amt = f"{e['现金流出']:,.0f}元"
                extra = f"({e['现金流类型']})" if e['现金流类型'] else ""
                items.append(f"{op} {e['产品']}{extra} {amt}")
...
# 渲染未来5日待办提醒（从各产品流动性表读取）
render_upcoming_reminders(sheets_data)
...
if not all_products:
    ...
    for tab, selected_product in zip(tabs, all_products):
        with tab:
            ...
            with col_edit:
                if st.button("⚙️ 编辑参数", key=f"edit_params_{selected_product}", use_container_width=True):
            ...
            if _from_session:
                ...
                if original_session_key in st.session_state:
                    ...
                    orig_start_idx = None
                    for row_i in range(len(original_df)):
                        if pd.notna(original_df.iloc[row_i]["日期"]) and original_df.iloc[row_i]["日期"] >= cutoff_date:
                            orig_start_idx = row_i
                            break
                ...
                with col_tool3:
                    if st.button("📋 复制末行", key=f"dup_{selected_product}"):
                        if len(edited_df) > 0:
                            ...
                            st.rerun()
                with col_tool4:
                    st.caption(f"共 {len(edited_df)} 行 | 总流入 ¥{edited_df['现金流入'].sum():,.2f} | 总流出 ¥{edited_df['现金流出'].sum():,.2f}")
